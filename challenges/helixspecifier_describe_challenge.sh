#!/usr/bin/env bash
# helixspecifier_describe_challenge.sh — round-273 anti-bluff
...
#
#   mutate:    sets HELIXSPECIFIER_MUTATE_RUNNER=1 which inverts
#              invariant 3 inside the runner. The runner MUST
...
MODE="${1:-normal}"
echo "=== HelixSpecifier Describe Challenge (round-273) ==="
echo "  mode=${MODE}"
...
case "${MODE}" in
    normal)
        unset HELIXSPECIFIER_MUTATE_RUNNER
        ...
        ;;
    mutate)
        export HELIXSPECIFIER_MUTATE_RUNNER=1
        out="$(go run ./challenges/runner/ 2>&1)"
        rc=$?
        echo "${out}" | tail -10
        if [[ "${rc}" -eq 0 ]]; then
